...
X_train.to_csv("kg/kg_train.tsv", sep="\t", header=False, index=False)
X_test.to_csv("kg/kg_test.tsv", sep="\t", header=False, index=False)
X_val.to_csv("kg/kg_valid.tsv", sep="\t", header=False, index=False)
...
for n, (train_index, test_index) in enumerate(kf.split(X, y=y)):
    ...
    X_train.to_csv(f"kg/kg_train_fold{n}.tsv", sep="\t", header=False, index=False)
    X_test.to_csv(f"kg/kg_test_fold{n}.tsv", sep="\t", header=False, index=False)

# Create full dataset
df.to_csv("kg/kg_full.tsv", sep="\t", index=False, header=False)
